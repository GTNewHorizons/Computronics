...
public class TileCipherBlockAdvanced extends TileEntityPeripheralBase {
    ...
    private static int checkPrime(int number, int i) {
        if (!isPrime(number)) {
            throw new IllegalArgumentException(String.format("bad argument #%s (prime expected, got %s)", i, number));
        } else {
            return number;
        }
    }
    ...
    private static Map<Integer, String> checkValidKey(Map<?, ?> map, int index) {
        ...
        throw new IllegalArgumentException(String.format("bad argument #%s (no valid RSA key)", index));
    }
        ...
        if (("prime").equals(publicKey.get(3))) {
            ...
            if (n.toByteArray().length < messageBytes.length) {
                throw new IllegalArgumentException(
                        "key is too small, needs to have a bit length of at least " + messageBytes.length
                                + ", but only has "
    ...
    private Object[] decrypt(Map<Integer, String> privateKey, byte[] messageBytes) throws Exception {
        ...
        if (("prime").equals(privateKey.get(3))) {
            ...
            if (n.toByteArray().length < decodedBytes.length) {
                throw new IllegalArgumentException(
                        "key is too small, needs to have a bit length of at least " + decodedBytes.length
                                + ", but only has "
                                + n.toByteArray().length);
            }
    ...
    @Callback(
            doc = "function([keylength:number]):keygen; Creates the key generator from two random prime numbers (optionally with given key length)",
            direct = true,
            limit = 1)
    @Optional.Method(modid = Mods.OpenComputers)
    public Object[] createRandomKeySet(Context c, Arguments a) {
        ...
        Object[] result = this
                .tryConsumeEnergy(new Object[] { val }, Config.CIPHER_KEY_CONSUMPTION, "createRandomKeySet");
        c.pause(0.5);
        return result;
    }

    @Callback(
            doc = "function(num1:number, num2:number):keygen; Creates the key generator from the two given prime numbers",
            direct = true,
            limit = 1)
    @Optional.Method(modid = Mods.OpenComputers)
    public Object[] createKeySet(Context c, Arguments a) {
        RSAValue val = new RSAValue();
        val.startCalculation(checkPrime(a.checkInteger(0), 0), checkPrime(a.checkInteger(1), 1));
        Object[] result = new Object[] { val };
        return this.tryConsumeEnergy(result, Config.CIPHER_KEY_CONSUMPTION, "createKeySet");
    }

    @Callback(
            doc = "function(message:string, publicKey:table):string; Encrypts the specified message using the specified public RSA key",
            direct = true,
            limit = 1)
    @Optional.Method(modid = Mods.OpenComputers)
    public Object[] encrypt(Context c, Arguments a) throws Exception {
        byte[] message = a.checkByteArray(0);
        Object[] result = this.encrypt(checkValidKey(a.checkTable(1), 1), message);
        return this.tryConsumeEnergy(result, Config.CIPHER_WORK_CONSUMPTION + 0.2 * message.length, "encrypt");
    }

    @Callback(
            doc = "function(message:string, privateKey:table):string; Decrypts the specified message using the specified RSA key",
            direct = true,
            limit = 1)
    @Optional.Method(modid = Mods.OpenComputers)
    public Object[] decrypt(Context c, Arguments a) throws Exception {
        byte[] message = a.checkByteArray(0);
        Object[] result = this.decrypt(checkValidKey(a.checkTable(1), 1), message);
        ...
        try {
            switch (method) {
                    ...
                    if (arguments.length > 0) {
                        if (!(arguments[0] instanceof Number)) {
                            throw new LuaException(
                                    "first argument needs to be a number, or there must not be any arguments");
                        }
                ...
                case 1: {
                    ...
                    RSAValue val = new RSAValue();
                    val.startCalculation(
                            checkPrime(((Number) arguments[0]).intValue(), 0),
