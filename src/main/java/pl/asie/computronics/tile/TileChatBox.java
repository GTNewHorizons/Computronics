...
public class TileChatBox extends TileEntityPeripheralBase implements IChatListener {
    ...
    public boolean isCreative() {
        return Config.CHATBOX_CREATIVE && worldObj != null
                && worldObj.blockExists(xCoord, yCoord, zCoord)
    ...
    public void receiveChatMessage(ServerChatEvent event) {
        ...
        if (!Config.CHATBOX_MAGIC && !isCreative()
                && (event.player.worldObj != this.worldObj
                        || event.player.getDistanceSq(xCoord, yCoord, zCoord) > distance * distance)) {
            return;
        }
    ...
    public void eventCC(ServerChatEvent event) {
        if (attachedComputersCC != null) {
            for (IComputerAccess computer : attachedComputersCC) {
                computer.queueEvent(
                        "chat_message",
                        new Object[] { computer.getAttachmentName(), event.username, event.message });
            }
        }
    }
    ...
    protected OCUtils.Device deviceInfo() {
        return new OCUtils.Device(DeviceClass.Multimedia, "Chat interface", OCUtils.Vendors.NSA, "[CLASSIFIED]");
    }
    // OpenComputers API

    @Callback(
            doc = "function(text:string [, distance:number]):boolean; "
    ...
    public Object[] say(Context context, Arguments args) {
        ...
        if (args.count() >= 1) {
            ...
            if (args.isString(0)) {
                ChatBoxUtils.sendChatMessage(
                        this,
                        d,
                        name.length() > 0 ? name : Config.CHATBOX_PREFIX,
                        args.checkString(0),
                        !hasDistance && (Config.CHATBOX_MAGIC || isCreative));
    ...
    @Callback(
            doc = "function(distance:number):number; Sets the distance of the chat box. Returns the new distance",
            direct = true)
    @Optional.Method(modid = Mods.OpenComputers)
            ...
            case 0: { // say
                if (arguments.length >= 1 && arguments[0] instanceof String) {
                    ...
                    if (arguments.length >= 2 && arguments[1] instanceof Double) {
                        d = isCreative ? ((Double) arguments[1]).intValue()
                                : Math.min(Config.CHATBOX_DISTANCE, ((Double) arguments[1]).intValue());
                        if (d <= 0) {
                            d = distance;
                        }
                        hasDistance = true;
