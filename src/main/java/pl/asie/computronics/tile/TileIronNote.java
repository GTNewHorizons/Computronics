...
public class TileIronNote extends TileEntityPeripheralBase implements IBundledTile, IBundledUpdatable, IConnectable {
    ...
    @Callback(
            direct = true,
            limit = 10,
            doc = "function([instrument:number or string,] note:number [, volume:number]); "
            ...
            if (args.count() >= 2 && args.isInteger(1)) {
                if (args.isInteger(0)) {
                    task = NoteUtils.playNote(
                            ...
                            args.checkInteger(0),
                            args.checkInteger(1),
                            NoteUtils.toVolume(3, args.optDouble(2, 1.0D)));
                ...
                task = NoteUtils.playNote(
                        ...
                        -1,
                        args.checkInteger(0),
                        NoteUtils.toVolume(2, args.optDouble(1, 1.0D)));
            }
        }
        if (task != null) {
            synchronized (noteBuffer) {
            ...
            if (arguments.length >= 1) {
                if (arguments.length >= 2 && (arguments[1] instanceof Double)) {
                    if (arguments[0] != null) {
                        if (arguments[0] instanceof Double) {
                            task = NoteUtils.playNote(
                                    ...
                                    NoteUtils.toVolume(arguments, 2));
                    ...
                    task = NoteUtils.playNote(
                            ...
                            -1,
                            ((Double) arguments[0]).intValue(),
                            NoteUtils.toVolume(arguments, 1));
                }
            }
            if (task != null) {
                synchronized (noteBuffer) {
