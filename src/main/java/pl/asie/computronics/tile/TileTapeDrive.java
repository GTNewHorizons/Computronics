...
public class TileTapeDrive extends TileEntityPeripheralBase implements IInventoryProvider, IAudioSource {
    ...
    public static void initCCFilesystem() {
        if (cc_fs == null) {
            cc_fs = ComputronicsFileMount.createMount(
                    Computronics.class,
                    Mods.Computronics,
                    "lua/peripheral/tape_drive/programs/tape_drive");
            if (cc_fs == null) {
                Computronics.log.error("Unable to create ComputerCraft mount for tape drive programs.");
            }
        }
        if (cc_fs_autorun == null) {
            cc_fs_autorun = ComputronicsFileMount
                    .createMount(Computronics.class, Mods.Computronics, "lua/peripheral/tape_drive/autorun/tape_drive");
            if (cc_fs == null) {
    ...
    protected OCUtils.Device deviceInfo() {
        return new OCUtils.Device(DeviceClass.Tape, "Tape drive", OCUtils.Vendors.Yanaki, "DFPWM 1");
    }
    ...
    protected void sendState() {
        ...
        try {
            Packet packet = Computronics.packet.create(PacketType.TAPE_GUI_STATE.ordinal()).writeTileLocation(this)
    ...
    public boolean isEnd() {
        return state.getStorage() == null
                || state.getStorage().getPosition() + state.packetSize > state.getStorage().getSize();
    }
    ...
    @Callback(
            doc = "function(length:number):number; Seeks the specified amount of bytes on the tape. "
    ...
    @Callback(
            doc = "function([length:number]):string; "
    ...
    @Callback(
            doc = "function(speed:number):boolean; Sets the speed of the tape drive. Needs to be beween 0.25 and 2. Returns true on success")
    @Optional.Method(modid = Mods.OpenComputers)
    ...
    public String[] getMethodNames() {
        return new String[] { "isEnd", "isReady", "getSize", "getLabel", "getState", "setLabel", "setSpeed",
                "setVolume", "seek", "read", "write", "play", "stop", "getPosition" };
    }
