...
public class TileRadar extends TileEntityPeripheralBase implements IBatteryProvider {
    ...
    public TileRadar() {
        super("radar", 0);
        this.registerBattery(
                new BatteryBasic(
                        EnergyConverter
                                .convertEnergy(Config.RADAR_ENERGY_COST_OC * Config.RADAR_RANGE * 3.5, "OC", "RF")));
    }
    ...
    protected OCUtils.Device deviceInfo() {
        return new OCUtils.Device(DeviceClass.Multimedia, "Radar", OCUtils.Vendors.Trumbour, "Detectotron M1");
    }
    ...
    public Object[] getItems(Context context, Arguments args) {
        ...
        if (((Connector) node()).tryChangeBuffer(0 - energyNeeded) || extractFromBattery(energyNeeded)) {
            AxisAlignedBB bounds = getBounds(distance);
            entities.addAll(
                    RadarUtils.getItems(worldObj, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, bounds, EntityItem.class));
            context.pause(0.5);
        }
        return new Object[] { TableUtils.convertSetToMap(entities) };
    }
