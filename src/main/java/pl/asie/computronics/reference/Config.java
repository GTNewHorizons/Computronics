...
public class Config {
    ...
    public void preInit() {
        ...
        // Camera
        CAMERA_DISTANCE = config
                .getInt("maxDistance", "camera", 32, 16, 256, "The maximum camera distance, in blocks.");

        // Chat Box
        CHATBOX_CREATIVE = config.getBoolean("enableCreative", "chatbox", true, "Enable Creative Chat Boxes.");
        CHATBOX_DISTANCE = config
                .getInt("maxDistance", "chatbox", 40, 4, 32767, "The maximum chat box distance, in blocks.");
        CHATBOX_MAGIC = config.getBoolean(
                "makeMagical",
                "chatbox",
                false,
                "Make the normal Chat Box have no range limit and work interdimensionally.");
        CHATBOX_PREFIX = config.getString("prefix", "chatbox", "ChatBox", "The Chat Box's default prefix.");

        // Cipher Block
        CIPHER_CAN_LOCK = config
                .getBoolean("canLock", "cipherblock", true, "Decides whether Cipher Blocks can or cannot be locked.");

        SOUND_SAMPLE_RATE = config.getInt(
                "soundSampleRate",
                "sound.client",
                44100,
                0,
                Integer.MAX_VALUE,
                "The sample rate used for generating sounds. Modify at your own risk.");
        SOUND_VOLUME = (byte) config
                .getInt("soundVolume", "sound.client", 127, 0, Byte.MAX_VALUE, "The base volume of generated sounds.");
        SOUND_RADIUS = config
                .getInt("soundRadius", "sound.client", 24, 0, 64, "The radius in which generated sounds can be heard.");

        TTS_ENABLED = config.getBoolean(
                "enableTextToSpeech",
                "tts",
                true,
                "Enable Text To Speech. To use it, install MaryTTS, a language and a corresponding voice into the marytts directory of your minecraft instance. For installation instructions, see http://wiki.vex.tty.sh/wiki:computronics:mary");
        TTS_MAX_LENGTH = config.getInt(
                "maxPhraseLength",
                "tts",
                300,
                0,
                100000,
                "The maximum number of text bytes the speech box can process at a time.");
        ...
        if (Mods.isLoaded(Mods.OpenComputers)) {
            ...
            CIPHER_ENERGY_STORAGE = convertRFtoOC(
                    config.getFloat(
                            ...
                            0.0f,
                            100000.0F,
                            "How much energy the Advanced Chipher Block can store"));
            CIPHER_KEY_CONSUMPTION = convertRFtoOC(
                    config.getFloat(
                            "cipherKeyConsumption",
                            "power",
                            16000.0f,
                            0.0f,
                            100000.0F,
                            "How much energy the Advanced Cipher Block should consume for creating a key set"));
            CIPHER_WORK_CONSUMPTION = convertRFtoOC(
                    config.getFloat(
                            "cipherWorkConsumption",
                            "power",
                            160.0f,
                            0.0f,
                            100000.0f,
                            "How much base energy the Advanced Cipher Block should consume per encryption/decryption task. It will consume this value + 2*(number of characters in message)"));
            ...
            if (OC_CARD_SOUND) {
                SOUND_CARD_MAX_DELAY = config.getInt(
                        "ocSoundCardMaxDelay",
                        "sound",
                        SOUND_CARD_MAX_DELAY,
                        0,
                        Integer.MAX_VALUE,
                        "Maximum delay allowed in a sound card's instruction queue, in milliseconds");
                SOUND_CARD_QUEUE_SIZE = config.getInt(
                        "ocSoundCardQueueSize",
                        "sound",
                        SOUND_CARD_QUEUE_SIZE,
                        0,
                        Integer.MAX_VALUE,
                        "Maximum  number of instructons allowed in a sound cards instruction queue. This directly affects the maximum size of the packets sent to the client.");
                SOUND_CARD_CHANNEL_COUNT = config.getInt(
                        "ocSoundCardChannelCount",
                        "sound",
                        SOUND_CARD_CHANNEL_COUNT,
                        1,
                        65536,
                        "The number of audio channels each sound card has.");
            }

            // Particle Card
            FX_ENERGY_COST = convertRFtoOC(
                    config.getFloat(
                            "ocParticleCardCostPerParticle",
                            "power",
                            2.0f,
                            0.0f,
                            10000.0f,
                            "How much energy 1 particle emission should take. Multiplied by the distance to the target."));
            // Spoofing Card
            SPOOFING_ENERGY_COST = convertRFtoOC(
                    config.getFloat(
                            "ocSpoofingCardCostPerMessage",
                            "power",
                            2.0f,
                            0.0f,
                            10000.0f,
                            "How much energy sending one spoofed message should take"));
            // Beep Card
            BEEP_ENERGY_COST = convertRFtoOC(
                    config.getFloat(
                            "ocBeepCardCostPerSound",
                            "power",
                            10.0f,
                            0.0f,
                            10000.0f,
                            "How much energy a single beep will cost for 1 second"));
            // Sound Card
            SOUND_CARD_ENERGY_COST = convertRFtoOC(
                    config.getFloat(
                            "ocSoundCardCostPerSecond",
                            "power",
                            10.0f,
                            0.0f,
                            10000.0f,
                            "How much energy the sound card will consume per second of processed sound."));
            // Colorful Upgrade
            COLORFUL_UPGRADE_COLOR_CHANGE_COST = convertRFtoOC(
                    config.getFloat(
                            "ocColorfulUpgradeColorChangeCost",
                            "power",
                            2.0f,
                            0.0f,
                            10000.0f,
                            "How much energy changing the color of the Colorful Upgrade will cost"));

            // Rack Mountables
            LIGHT_BOARD_COLOR_CHANGE_COST = convertRFtoOC(
                    config.getFloat(
                            "ocLightBoardColorChangeCost",
                            "power",
                            2.0f,
                            0.0f,
                            10000.0f,
                            "How much energy changing the color or state of a Light Board's light will cost"));
            LIGHT_BOARD_COLOR_MAINTENANCE_COST = convertRFtoOC(
                    config.getFloat(
                            "ocLightBoardColorMaintenanceCost",
                            "power",
                            0.2f,
                            0.0f,
                            10000.0f,
                            "How much energy will be consumed per tick to keep a Light Board's light running. Note that this value is consumed for each active light on the board."));
            BOOM_BOARD_MAINTENANCE_COST = convertRFtoOC(
                    config.getFloat(
                            "ocBoomBoardMaintenanceCost",
                            "power",
                            0.2f,
                            0.0f,
                            10000.0f,
                            "How much energy will be consumed per tick to keep a Server Self-Destructor active."));
            RACK_CAPACITOR_CAPACITY = convertRFtoOC(
                    config.getFloat(
                            "ocRackCapacitorCapacity",
                            "power",
                            7500f,
                            0.0f,
                            10000.0f,
                            "How much energy a Rack Capacitor can store."));
            SWITCH_BOARD_MAINTENANCE_COST = convertRFtoOC(
                    config.getFloat(
                            "ocSwitchBoardMaintenanceCost",
                            "power",
                            0.2f,
                            0.0f,
                            10000.0f,
                            "How much energy will be consumed per tick to keep a Switch Board's switch active. Note that this value is consumed for each active switch on the board."));

            if (Mods.isLoaded(Mods.Railcraft)) {
                LOCOMOTIVE_RELAY_BASE_POWER = convertRFtoOC(
                        config.getFloat(
                                "locomotiveRelayBasePower",
                                "power.railcraft",
                                20.0f,
                                0.0f,
                                10000.0f,
                                "How much base energy the Locomotive Relay consumes per operation"));
            }

            NON_OC_RECIPES = config.getBoolean(
                    "easyRecipeMode",
                    "recipes",
                    false,
                    "Set this to true to make some recipes not require OpenComputers blocks and items");

            if (Mods.hasVersion(Mods.Forestry, Mods.Versions.Forestry)) {
                FORESTRY_BEES = config.getBoolean(
                        "opencomputersBees",
                        "enable.forestry",
                        true,
                        "Set this to false to disable Forestry bee species for OpenComputers");
            }
            if (Mods.isLoaded(Mods.BuildCraftTransport) && Mods.isLoaded(Mods.BuildCraftCore)) {
                BUILDCRAFT_STATION = config.getBoolean(
                        "droneDockingStation",
                        "enable.buildcraft",
                        true,
                        "Set this to false to disable the Drone Docking Station for OpenComputers");
            }
            if (Mods.isLoaded(Mods.GregTech)) GT_COORDINATES = config.getBoolean(
                    "GregtechMachineCoordinates",
                    "gregtech",
                    true,
                    "Set this to false to disable the coordinates function for Gregtech machines");
        }

        if (Mods.isLoaded(Mods.ComputerCraft)) {
            if (Mods.isLoaded(Mods.OpenPeripheral)) {
                CC_OPEN_MULTI_PERIPHERAL = config.getBoolean(
                        "openMultiPeripheral",
                        "computercraft.multiperipheral",
                        true,
                        "Set this to false to disable MultiPeripheral compatibility with OpenPeripheral peripherals");
            }
            CC_ALL_MULTI_PERIPHERALS = config.getBoolean(
                    "allMultiPeripherals",
                    "computercraft.multiperipheral",
                    true,
                    "Set this to true to fix multiple mods adding peripherals to the same block not working");
            CC_ALWAYS_FIRST = config.getBoolean(
                    "alwaysFirstPeripheral",
                    "computercraft.multiperipheral",
                    true,
                    "If this is true, the Computronics MultiPeripheral system will almost always be the one recognized by ComputerCraft");
            ...
            if (CC_OPEN_MULTI_PERIPHERAL && CC_ALL_MULTI_PERIPHERALS && CC_ALWAYS_FIRST) {
                ...
                Computronics.log.info(
                        "Multiple mods registering peripherals for the same block now won't be a problem anymore.");
            }
        }
        ...
        RADAR_ONLY_DISTANCE = config.getBoolean(
                "onlyOutputDistance",
                "radar",
                true,
                "Stop Radars from outputting X/Y/Z coordinates and instead only output the distance from an entity.");

        // Particles
        FX_RANGE = config.getInt(
                "particleRange",
                "particles",
                FX_RANGE,
                -1,
                65536,
                "The maximum range of particle-emitting devices. Set to -1 to make it work over any distance.");

        // Tape Drive
        TAPEDRIVE_BUFFER_MS = config.getInt(
                "audioPreloadMs",
                "tapedrive",
                750,
                500,
                10000,
                "The amount of time (in milliseconds) used for pre-buffering the tape for audio playback. If you get audio playback glitches in SMP/your TPS is under 20, RAISE THIS VALUE!");
        TAPEDRIVE_DISTANCE = config.getInt(
                "hearingDistance",
                "tapedrive",
                24,
                0,
                64,
                "The distance up to which Tape Drives can be heard.");
        TAPE_LENGTHS = config.getString(
                "tapeLengths",
                "tapedrive",
                "4,8,16,32,64,2,6,16,128,128",
                "The lengths of the computronics tapes. Should be 10 numbers separated by commas");

        PORTABLE_TAPEDRIVE_DISTANCE = config.getInt(
                "hearingDistance",
                "tapedrive.portable",
                8,
                0,
                64,
                "The distance up to which Portable Tape Drives can be heard.");

        // General
        REDSTONE_REFRESH = config.getBoolean(
                "enableTickingRedstoneSupport",
                "general",
                true,
                "Set whether some machines should stop being tickless in exchange for redstone output support.");
        ...
        RADAR_ENERGY_COST_OC = convertRFtoOC(
                config.getFloat(
                        ...
                        0.0f,
                        10000.0f,
                        "How much energy each 1-block distance takes by OpenComputers radars."));

        // Railcraft integration
        if (Mods.isLoaded(Mods.Railcraft)) {
            LOCOMOTIVE_RELAY_RANGE = (double) config.getInt(
                    "locomotiveRelayRange",
                    "railcraft",
                    128,
                    0,
                    512,
                    "The range of Locomotive Relays in Blocks.");
            LOCOMOTIVE_RELAY_CONSUME_CHARGE = config.getBoolean(
                    "locomotiveRelayConsumeCharge",
                    "railcraft",
                    true,
                    "If true, the Locomotive Relay will consume"
                            + "a little bit of Railcraft charge in the locomotive everytime it is accessing the locomotive");
            TICKET_MACHINE_CONSUME_RF = config.getBoolean(
                    "ticketMachineConsumeCharge",
                    "railcraft",
                    true,
                    "If true, the Ticket Machine will" + "require a little bit of RF to print tickets");
        }

        // GregTech recipe mode
        if (Mods.isLoaded(Mods.GregTech)) {
            GREGTECH_RECIPES = config
                    .getBoolean("gtRecipeMode", "recipes", true, "Set this to true to enable GregTech-style recipes");
        }
