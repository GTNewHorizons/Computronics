...
public class ItemTape extends Item implements IItemTapeStorage, IMedia, IMediaProvider, IItemWithDocumentation {
    ...
    public ItemTape(String lengths) {
         ...
         * " [default: 4,8,16,32,64,2,6,16,128,128]"); Config.TAPE_LENGTHS = "4,8,16,32,64,2,6,16,128,128"; lengths =
         * Config.TAPE_LENGTHS; l = lengths.split(","); }
         */
        ...
        for (int i = 0; i < TAPE_COUNT; i++) {
            if (i < l.length) {
                try {
                    ...
                    Computronics.log.error(
                            "Property 'tapedrive.tapeLengths' in computronics.cfg contains entry"
                ...
                Computronics.log.warn(
                        "Property 'tapedrive.tapeLengths' contains too few entries, setting entry "
                                + String.valueOf(i + 1)
                                + " to "
                                + DEFAULT_LENGTHS[i]);
                sizes[i] = DEFAULT_LENGTHS[i] * L_MINUTE;
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List<String> text, boolean par4) {
        ...
        switch (stack.getItemDamage()) {
            ...
            case 9: {
                String[] local = StringUtil.localize("tooltip.computronics.tape.ig").replace("\\n", "\n").split("\\n");
                for (String s : local) {
