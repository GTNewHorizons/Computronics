...
public class ItemPortableTapeDrive extends Item implements IItemWithDocumentation {
    ...
    public void registerIcons(IIconRegister r) {
        ...
        for (int i = 0; i < stateIcons.length; i++) {
            stateIcons[i] = r.registerIcon(
                    "computronics:portable_tape_drive_" + State.VALUES[i].name().toLowerCase(Locale.ENGLISH));
        }
    }
    ...
    public boolean onEntityItemUpdate(EntityItem entity) {
        PortableTapeDrive drive = PortableDriveManager.INSTANCE
                .getOrCreate(entity.getEntityItem(), entity.worldObj.isRemote);
        drive.resetTime();
    ...
    public void onUpdate(ItemStack stack, World world, Entity carrier, int slot, boolean isSelected) {
        ...
        /*
         * if(!world.isRemote) { return; } PortableTapeDrive drive = PortableDriveManager.INSTANCE.getOrCreate(stack,
         * world.isRemote); drive.resetTime(); drive.updateCarrier(carrier, stack); drive.update();
         */
    }
