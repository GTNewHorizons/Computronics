...
public class ActionParameterLampColor extends ComputronicsParameter {
    ...
    public String getDescription() {
        if (this.stack != null) {
            return StringUtil.localizeAndFormat(
                    "tooltip.computronics.gate.action.lamp_color." + ItemDye.field_150923_a[this.color],
                    stack.stackSize);
        }
        return StringUtil.localizeAndFormat(
                "tooltip.computronics.gate.action.lamp_color." + ItemDye.field_150923_a[this.color],
    ...
    @Override
    public void onClick(IStatementContainer container, IStatement statement, ItemStack stack,
            StatementMouseClick mouseClick) {
        ...
        if (this.stack == null && button == 0 && !shift) {
            ...
            this.stack = new ItemStack(
                    Items.dye,
                    (stack != null && stack.stackSize >= 1 && button != 2) ? stack.stackSize : button == 0 ? 10 : 31,
                    color);
