...
public class DriverDockingUpgrade extends ManagedEnvironment implements DeviceInfo {
    ...
    public DriverDockingUpgrade(Drone drone) {
        this.drone = drone;
        this.setNode(Network.newNode(this, Visibility.Neighbors).withComponent("docking").create());
    }
        ...
        if ((isDocking || isDocked) && pipevec != null) {
            ...
            if (tile instanceof IPipeTile) {
                ...
                if (pipe.getPipePluggable(side) != null
                        && pipe.getPipePluggable(side) instanceof DroneStationPluggable) {
                    ...
                    double targetY = pipe != null ? pipe.y() + 1.5
                            : pipevec != null ? pipevec[1] + 1.5 : target.yCoord + 0.5;
    ...
    private int injectItem(TileGenericPipe pipe, ItemStack stack, boolean doAdd, ForgeDirection from, EnumColor color) {
        if (!pipe.pipe.inputOpen(from)) {
            return 0;
        } else if (BlockGenericPipe.isValid(pipe.pipe) && pipe.pipe.transport instanceof PipeTransportItems
                && pipe.getPipePluggable(from) != null) {
                    if (doAdd) {
                        Position position = new Position(
                                (double) pipe.xCoord + 0.5D,
                                (double) pipe.yCoord + 0.02D,
                                (double) pipe.zCoord + 0.5D,
                                from.getOpposite());
    ...
    @Callback(
            doc = "function(slot:number[,maxAmount:number[,color:number]]):number; drops an item into the attached pipe if docked; Returns the amount of items dropped on success; Allows coloring the item if the drone is tier 2")
    public Object[] dropItem(Context context, Arguments args) {
        ...
        if (stack != null && stack.getItem() != null) {
            ...
            int rejected = stack.stackSize - injectItem(
                    (TileGenericPipe) pipe,
                    stack,
                    true,
                    side,
                    args.count() > 2 && drone.tier() > 0 ? EnumColor.fromId(15 - args.checkInteger(2)) : null);
    ...
    @Callback(
            doc = "function():boolean; Makes the drone start docking with a docking station; Always tries to dock with a station below it first")
    ...
    public void load(NBTTagCompound nbt) {
        ...
        if ((isDocked || isDocking)) {
            pipevec = new int[] { nbt.getInteger("drone:dockX"), nbt.getInteger("drone:dockY"),
    ...
    public Map<String, String> getDeviceInfo() {
        if (deviceInfo == null) {
            return deviceInfo = new OCUtils.Device(
                    ...
                    "DroneDock 233-B Deluxe").deviceInfo();
        }
        return deviceInfo;
    }
}
