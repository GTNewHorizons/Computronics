...
public class DriverSpatialIOPort {
    ...
    private static Object[] getInformation(TileSpatialIOPort tile) {
        ...
        try {
            ...
            if ((spc.hasRegion()) && (spc.isValidRegion())) {
                ...
                map.put(
                        "canTrigger",
                        SpatialIOUtil.isSpatialCell(cell) && tile.getStackInSlot(1) == null
                                && Math.abs(pr - req) < req * 0.001D);
            } else {
    ...
    public static class OCDriver extends DriverSidedTileEntity {
        ...
        public static class InternalManagedEnvironment extends ManagedEnvironmentOCTile<TileSpatialIOPort> {
            ...
            @Callback(
                    doc = "function():boolean; Puts the cell in the IO Port's output slot into its input slot if possible; returns true on success")
            public Object[] swapCell(Context c, Arguments a) {
                return DriverSpatialIOPort.swapCell(tile);
            }

            @Callback(
                    doc = "function():boolean; Checks whether the cell in the IO Port's output slot can be moved into its input slot")
            public Object[] canSwapCell(Context c, Arguments a) {
                return DriverSpatialIOPort.canSwapCell(tile);
            }

            @Callback(
                    doc = "function():number, number, number; Returns the size of the storage cell in the input slot (x, y and z size)")
    ...
    public static class CCDriver extends CCMultiPeripheral<TileSpatialIOPort> {
        ...
        public String[] getMethodNames() {
            return new String[] { "trigger", "canTrigger", "swapCell", "canSwapCell", "getCellSize", "getMaxCellSize",
                    "getInformation" };
        }
