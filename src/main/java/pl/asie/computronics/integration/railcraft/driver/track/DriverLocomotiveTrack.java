...
public class DriverLocomotiveTrack {
    ...
    private static Object[] getMode(TrackLocomotive tile) {
        ...
        tile.writeToNBT(data);
        return new Object[] {
                data.hasKey("mode") ? Math.abs(data.getByte("mode") % EntityLocomotive.LocoMode.VALUES.length - 2)
                        : null };
    }
    ...
    public static class OCDriver extends DriverSidedTileEntity {
        ...
        public static class InternalManagedEnvironment extends ManagedEnvironmentOCTile<TrackLocomotive> {
            ...
            @Callback(
                    doc = "function(mode:number):boolean; sets the Locomotive mode to the specified value; returns true on success")
        ...
        public ManagedEnvironment createEnvironment(World world, int x, int y, int z, ForgeDirection side) {
            return new InternalManagedEnvironment(
                    (TrackLocomotive) ((TileTrack) world.getTileEntity(x, y, z)).getTrackInstance());
        }
    }
    ...
    public static class CCDriver extends CCMultiPeripheral<TrackLocomotive> {
        ...
        public IMultiPeripheral getPeripheral(World world, int x, int y, int z, int side) {
            ...
            if (te != null && te instanceof TileTrack
                    && ((TileTrack) te).getTrackInstance() instanceof TrackLocomotive) {
