...
@Optional.InterfaceList({
        ...
        @Optional.Interface(
                iface = "mods.railcraft.common.plugins.buildcraft.triggers.IAspectProvider",
                modid = Mods.Railcraft) })
public class TileDigitalReceiverBox extends TileDigitalBoxBase implements IReceiverTile, IAspectProvider {
    ...
    public void updateEntity() {
        ...
        if (worldObj.isRemote) {
            ...
            if ((this.receiver.getVisualAspect().isBlinkAspect())
    ...
    public void eventCC(String name, SignalAspect aspect) {
        if (attachedComputersCC != null) {
            for (IComputerAccess computer : attachedComputersCC) {
                computer.queueEvent(
                        "aspect_changed",
                        new Object[] { computer.getAttachmentName(), name, aspect.ordinal() + 1 });
            }
        }
    }
    ...
    @Callback(
            doc = "function(name:string):number; Returns the aspect currently received from a connected signal with the specified name. Returns nil and an error message on failure.",
            direct = true,
            limit = 32)
    @Optional.Method(modid = Mods.OpenComputers)
    ...
    @Callback(
            doc = "function():number; Returns the most restrictive aspect currently received from connected signals",
            direct = true,
            limit = 32)
    @Optional.Method(modid = Mods.OpenComputers)
    ...
    @Callback(
            doc = "function(name:string):number; Tries to remove any pairing to a signal with the specified name. Returns true on success.",
            direct = true,
            limit = 32)
    @Optional.Method(modid = Mods.OpenComputers)
    ...
    @Callback(
            doc = "function():table; Returns a list containing the name of every paired controller.",
            direct = true,
            limit = 32)
    @Optional.Method(modid = Mods.OpenComputers)
