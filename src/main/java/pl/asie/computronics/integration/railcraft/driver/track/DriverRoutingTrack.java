...
public class DriverRoutingTrack {
    ...
    public static class OCDriver extends DriverSidedTileEntity {
        ...
        public static class InternalManagedEnvironment extends ManagedEnvironmentOCTile<TrackRouting> {
            ...
            /*
             * @Callback(doc = "function():String; gets the current owner of the ticket inside the track") public
             * Object[] getOwner(Context c, Arguments a) { ItemStack ticket = ((TrackRouting) ((ITrackTile)
             * track).getTrackInstance()).getInventory().getStackInSlot(0); if(ticket != null && ticket.getItem()
             * instanceof ItemTicketGold) { if(!((TrackRouting) ((ITrackTile) track).getTrackInstance()).isSecure()) {
             * return new Object[] { ItemTicketGold.getOwner(ticket) }; } else { return new Object[] { false,
             * "routing track is locked" }; } } else { return new Object[] { false,
             * "there is no golden ticket inside the track" }; } }
             */
        }
        ...
        public ManagedEnvironment createEnvironment(World world, int x, int y, int z, ForgeDirection side) {
            return new InternalManagedEnvironment(
                    (TrackRouting) ((TileTrack) world.getTileEntity(x, y, z)).getTrackInstance());
        }
    }
