...
public class GuiTicketMachine extends GuiContainerRailcraft {
    ...
    private static final ToolTip printLockedToolTips = ToolTips
            .buildToolTip("tooltip.computronics.ticket.printLocked", 0);

    public GuiTicketMachine(InventoryPlayer inventory, TileTicketMachine tile, boolean maintenanceMode) {
        super(
                new ContainerTicketMachine(inventory, tile, maintenanceMode),
                "computronics:textures/gui/ticket_machine.png");
        this.tile = tile;
        this.maintenanceMode = maintenanceMode;
        this.ownerName = tile.getOwner().getName();
        this.lockedToolTips = ToolTips
                .buildToolTip("tooltip.computronics.ticket.locked", 500, "{owner}=" + this.ownerName);
        this.unlockedToolTips = ToolTips
                .buildToolTip("tooltip.computronics.ticket.unlocked", 500, "{owner}=" + this.ownerName);
        this.notownedToolTips = ToolTips
                .buildToolTip("tooltip.computronics.ticket.notowner", 500, "{owner}=" + this.ownerName);
        this.notmaintenanceToolTips = ToolTips
                .buildToolTip("tooltip.computronics.ticket.notmaintenance", 500, "{owner}=" + this.ownerName);
    }
    ...
    protected void drawGuiContainerForegroundLayer(int x, int y) {
        fontRendererObj.drawString(StringUtil.localize("container.inventory"), 8, ySize - 96 + 2, 0x404040);
        String name = StringUtil.localize(tile.getGuiLocalizationTag());
        fontRendererObj.drawString(name, (xSize / 2) - (fontRendererObj.getStringWidth(name) / 2), 4, 0x404040);
    }
        ...
        for (Widget widget : container.getElements()) {
            if (widget instanceof LockButtonWidget) {
                if (((LockButtonWidget) widget).accessible) {
                    ((LockButtonWidget) widget)
                            .setToolTip(tile.isLocked() ? this.lockedToolTips : this.unlockedToolTips);
                } else {
                    ((LockButtonWidget) widget).setToolTip(
                            ((ContainerTicketMachine) container).canLock ? this.notmaintenanceToolTips
