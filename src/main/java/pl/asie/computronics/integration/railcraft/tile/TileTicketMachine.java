...
public class TileTicketMachine extends TileEntityPeripheralBase
        implements IInventoryProvider, IOwnable, IBatteryProvider {
    ...
    private static final int paperSlot = 10, ticketSlot = 11;
    ...
    protected void sendLockChange() {
        try {
            ...
            Packet packet = Computronics.packet.create(PacketType.TICKET_SYNC.ordinal()).writeTileLocation(this)
                    .writeInt(i).writeInt(selectedSlot);
    ...
    public void updateEntity() {
        ...
        if (progress >= getMaxProgress()) {
            ...
            if (outputSlotStack != null) {
                if (!outputSlotStack.getItem().equals(currentTicket.getItem())
                        || !ItemStack.areItemStackTagsEqual(outputSlotStack, currentTicket)) {
                    return;
                }
                if (!outputSlotStack.isStackable() || outputSlotStack.stackSize >= outputSlotStack.getMaxStackSize()) {
                    return;
                }
        ...
        if (this.node() instanceof Connector) {
            ...
            if (power < 0) {
                return new Object[] { null, null,
                        power + ": "
                                + methodName
                                + ": not enough energy available: required"
                                + v
                                + ", found "
                                + ((Connector) node()).globalBuffer() };
            }
        }
        return null;
    }
    ...
    protected OCUtils.Device deviceInfo() {
        return new OCUtils.Device(DeviceClass.Printer, "Ticket machine", OCUtils.Vendors.Railcraft, "Dot matrix 3000");
    }
    ...
    public Object[] printTicket(int slot, int amount, boolean opencomputers) {
        ...
        ItemStack outputSlotStack = this.getStackInSlot(ticketSlot);
        if (outputSlotStack != null) {
            if (!outputSlotStack.getItem().equals(ticket.getItem())
                    || !ItemStack.areItemStackTagsEqual(outputSlotStack, ticket)) {
    ...
    @Callback(
            doc = "function([amount:number [, slot:number]]):boolean; Tries to print one or more tickets from the current or the specified ticket slot; amount may be nil; Returns true on success")
    @Optional.Method(modid = Mods.OpenComputers)
    ...
    @Callback(
            doc = "function(allowed:boolean):boolean; permits or prohibits manual printing; Returns true if manual printing is allowed")
    @Optional.Method(modid = Mods.OpenComputers)
    ...
    @Callback(
            doc = "function(allowed:boolean):boolean; permits or prohibits manually selecting a ticket; Returns true if manual ticket selection is allowed")
    @Optional.Method(modid = Mods.OpenComputers)
    ...
    @Callback(
            doc = "function([slot:number,] destination:string):string; Tries to set the destination of the currently selected or the specified ticket; Returns the new destination")
    @Optional.Method(modid = Mods.OpenComputers)
    ...
    @Callback(
            doc = "function([slot:number]):string; Returns the destination of the currently selected or the specified ticket")
    @Optional.Method(modid = Mods.OpenComputers)
    ...
    @Callback(
            doc = "function(slot:number):number; Sets the currently selected ticket slot; Returns the new selected slot")
    @Optional.Method(modid = Mods.OpenComputers)
            ...
            switch (method) {
                case 0: {
                    ...
                    return this.printTicket(
                            ...
                            arguments[0] != null ? ((Number) arguments[0]).intValue() : 1,
                            false);
                }
                case 1: {
