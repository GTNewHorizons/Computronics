...
public class DriverRoutingSwitch {
    ...
    private static Object[] setRoutingTableTitle(TileSwitchRouting tile, Object[] arguments) {
        if (tile.getInventory().getStackInSlot(0) != null
                && tile.getInventory().getStackInSlot(0).getItem() instanceof ItemRoutingTable) {
            if (!tile.isSecure()) {
                return new Object[] { RoutingTableUtil
                        .setRoutingTableTitle(tile.getInventory().getStackInSlot(0), (String) arguments[0]) };
            } else {
                return new Object[] { false, "routing switch is locked" };
            }
        }
    ...
    public static class OCDriver extends DriverSidedTileEntity {
        ...
        public static class InternalManagedEnvironment extends ManagedEnvironmentOCTile<TileSwitchRouting> {
            ...
            @Callback(
                    doc = "function():table; returns the full routing table inside the switch motor, or false and an error message if the table is empty or cannot be accessed")
            public Object[] getRoutingTable(Context c, Arguments a) {
                return DriverRoutingSwitch.getRoutingTable(tile);
            }

            @Callback(
                    doc = "function(routingTable:table):boolean; Sets the routing table inside the switch; argument needs to be a table with number indices and string values, every value being a new line, for a new page, use '{newline}' as a value; returns 'true' on success, 'false' and an error message otherwise")
            ...
            @Callback(
                    doc = "function(name:string):boolean; Sets the name of the routing table inside the switch motor; returns true on success")
