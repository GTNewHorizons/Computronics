...
public class ItemRelaySensor extends Item implements IItemWithPrefix {
    ...
    public boolean onItemUseFirst(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (player.isSneaking() && world.getTileEntity(x, y, z) != null
                && world.getTileEntity(x, y, z) instanceof TileLocomotiveRelay
                ...
                if (entity instanceof EntityLocomotiveElectric) {
                    if (!player.worldObj.blockExists(x, y, z)) {
                        player.addChatComponentMessage(
                                new ChatComponentTranslation("chat.computronics.sensor.noRelayDetected"));
                        return true;
                    }
                    if (entity.worldObj.getTileEntity(x, y, z) != null
                        ...
                        if (loco.dimension == relay.getWorldObj().provider.dimensionId) {
                            if (loco.getDistanceSq(relay.xCoord, relay.yCoord, relay.zCoord)
                                    <= Config.LOCOMOTIVE_RELAY_RANGE * Config.LOCOMOTIVE_RELAY_RANGE) {
                                relay.setLocomotive(loco);
                                player.addChatComponentMessage(
                                        new ChatComponentTranslation("chat.computronics.sensor.bound"));
                                player.swingItem();
                                player.destroyCurrentEquippedItem();
                            } else {
                                player.addChatComponentMessage(
                                        new ChatComponentTranslation("chat.computronics.sensor.tooFarAway"));
                            }
                        } else {
                            player.addChatComponentMessage(
                                    new ChatComponentTranslation("chat.computronics.sensor.wrongDim"));
                        }
                    } else {
                        player.addChatComponentMessage(
                                new ChatComponentTranslation("chat.computronics.sensor.noRelay"));
                    }
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List<String> text, boolean par4) {
        ...
        if (stack.hasTagCompound() && stack.getTagCompound().getBoolean("bound")) {
            ...
            int z = data.getInteger("relayZ");
            text.add(
                    EnumChatFormatting.AQUA + StringUtil.localizeAndFormat(
                            "tooltip.computronics.sensor.bound",
                            String.valueOf(x),
                            String.valueOf(y),
                            String.valueOf(z)));
