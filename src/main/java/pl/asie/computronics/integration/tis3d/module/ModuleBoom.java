...
public class ModuleBoom extends ComputronicsModule {
    ...
    private static final ResourceLocation ICON = new ResourceLocation(
            "computronics:textures/blocks/tis3d/module_boom.png");
    ...
    public void render(boolean enabled, float partialTicks) {
        ...
        alpha = enabled ? 0x44 + (int) ((maxAlpha - alpha) * ((Math.sin(
                (getCasing().getCasingWorld().getTotalWorldTime() + (hashCode() % speed) + partialTicks)
                        / ((double) speed)
                        * 2D
                        * Math.PI)
                + 1D) / 2D)) : 0x44;
