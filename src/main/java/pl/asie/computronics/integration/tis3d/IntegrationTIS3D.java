...
public class IntegrationTIS3D {
        ...
        if (Config.TIS3D_MODULE_COLORFUL || Config.TIS3D_MODULE_TAPE_READER || Config.TIS3D_MODULE_BOOM) {
            ...
            if (Computronics.proxy.isClient()) {
                ...
                MinecraftForgeClient
                        .registerItemRenderer(itemModules, new ComputronicsModuleRenderer().setIgnoreLighting(true));
            }
        }
     ...
     * @SubscribeEvent public void textureHook(TextureStitchEvent.Pre event) { if(event.map.getTextureType() == 0) {
     * for(Textures t : Textures.VALUES) { t.registerIcon(event.map); } } } enum Textures {
     * CASING("tis3d/casingModule"), TAPE_READER_OFF("tape_drive_front");
     * @SideOnly(Side.CLIENT) private IIcon icon; private final String location; public static final Textures[] VALUES =
     * values(); Textures(String location) { this.location = location; } public IIcon getIcon() { return icon; }
     * @SideOnly(Side.CLIENT) public void registerIcon(IIconRegister iconRegister) { this.icon =
     * iconRegister.registerIcon("computronics:" + location); } } }
     */
}
