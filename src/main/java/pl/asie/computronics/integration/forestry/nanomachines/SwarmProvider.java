...
public class SwarmProvider extends AbstractProvider {
    ...
    protected void writeBehaviorToNBT(Behavior behavior, NBTTagCompound nbt) {
        /*
         * if(behavior instanceof SwarmBehavior) { ((SwarmBehavior) behavior).writeToNBT(nbt); }
         */
    }
        ...
        if (behavior != null) {
            if (behavior.entity != null) {
                ...
                behavior.spawnNewEntity(
                        ...
                        player.posY + 2f,
                        player.posZ,
                        BeeManager.beeRoot.getMemberStack(
                                BeeManager.beeRoot.templateAsIndividual(BeeManager.beeRoot.getDefaultTemplate()),
                                EnumBeeType.QUEEN.ordinal()));
                swingItem(player, e);
            }
        } /*
           * else { Controller controller = Nanomachines.installController(player); boolean win = false; while(!win) {
           * Iterator<Behavior> behaviorSet = ((ControllerImpl)
           * controller).configuration().behaviorMap().keySet().iterator(); while(behaviorSet.hasNext()) { Behavior next
           * = behaviorSet.next(); if(next instanceof SwarmBehavior) { win = true; break; } } controller.reconfigure();
           * } }
           */
    }

    private void makeSwarm(double x, double y, double z, EntityPlayer player, IBeeHousing tile, Event e) {
        if (tile.getBeekeepingLogic() != null && tile.getBeeInventory() != null
        ...
        if (player != null && !player.worldObj.isRemote) {
            ...
            if (heldItem != null && heldItem.getItem() == IntegrationForestry.itemStickImpregnated) {
                if (player.isSneaking() && e.minecart instanceof IBeeHousing) {
                    makeSwarm(
                            e.minecart.posX,
                            e.minecart.posY + 0.25,
                            e.minecart.posZ,
                            player,
                            (IBeeHousing) e.minecart,
                            e);
                } else {
                    findTarget(player, e);
                }
    ...
    /*
     * @SubscribeEvent public void onPlayerTick(TickEvent.PlayerTickEvent e) { SwarmBehavior behavior =
     * getSwarmBehavior(e.player); if(behavior != null && !e.player.worldObj.isRemote && e.phase ==
     * TickEvent.PlayerTickEvent.Phase.START) { behavior.update(); } }
     */
    ...
    private SwarmBehavior getSwarmBehavior(EntityPlayer player) {
        ...
        /*
         * SwarmBehavior behavior = behaviors.get(player.getCommandSenderName()); if(behavior == null) { behavior = new
         * SwarmBehavior(player); behavior.onEnable(); behaviors.put(player.getCommandSenderName(), behavior); } return
         * behavior;
         */
    }
