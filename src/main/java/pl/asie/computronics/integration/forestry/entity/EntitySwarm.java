...
public class EntitySwarm extends EntityFlyingCreature implements IBeeHousing {
    ...
    public void onEntityUpdate() {
        ...
        if (!worldObj.isRemote) {
            if (player != null) {
                if (player.isDead || (player.worldObj.provider.dimensionId != this.worldObj.provider.dimensionId)
                        || this.isInsideOfMaterial(Material.water)
                        || (getAttackTarget() == null
                                && ((player.getDistanceSqToEntity(this) > 2500 && !canEntityBeSeen(player))
            ...
            /*
             * ArrayList<Entity> toKeep = new ArrayList<Entity>(); for(Entity member : swarmMembers) { if(member != null
             * && !member.isDead) { toKeep.add(member); } } swarmMembers = toKeep; int m = 10; int memberCountMax =
             * getAmplifier() * m; memberCountMax = Math.max(m, (int) (memberCountMax * (getHealth() /
             * getMaxHealth()))); if(swarmMembers.size() < memberCountMax) { for(int i = 0; i < (memberCountMax -
             * swarmMembers.size()); ++i) { double xPos = posX + (width / 2) + (worldObj.rand.nextDouble() *
             * (worldObj.rand.nextBoolean() ? 0.5 : -0.5)), yPos = posY + (height / 2) + (worldObj.rand.nextDouble() *
             * (worldObj.rand.nextBoolean() ? 0.5 : -0.5)), zPos = posZ + (width / 2) + (worldObj.rand.nextDouble() *
             * (worldObj.rand.nextBoolean() ? 0.5 : -0.5)); EntitySwarmFX member = new EntitySwarmFX(worldObj, xPos,
             * yPos, zPos, 0xF0F000, this); swarmMembers.add(member); Computronics.proxy.spawnSwarmParticle(member); } }
             */
            ...
            for (int i = 0; i < (memberCountMax); ++i) {
                double xPos = posX /* + (width / 2f) */ + (worldObj.rand.nextDouble()
                        * (worldObj.rand.nextBoolean() ? 0.5 : -0.5));
                double yPos = posY + (height / 2f)
                        + (worldObj.rand.nextDouble() * (worldObj.rand.nextBoolean() ? 0.5 : -0.5));
                double zPos = posZ /* + (width / 2f) */ + (worldObj.rand.nextDouble()
                        * (worldObj.rand.nextBoolean() ? 0.5 : -0.5));
                Computronics.proxy.spawnSwarmParticle(worldObj, xPos, yPos, zPos, getColor());
            }
            // EntityBeeFX member = new EntityBeeFX(worldObj, posX /*+ (width / 2f)*/, posY + (height / 2f), posZ /*+
            // (width / 2f)*/, 0f, 0f, 0f, 0xF0F000);
    ...
    protected void updateEntityActionState() {
        ...
        if (target != null) {
            if (target.isDead || (target == player && !aggressive)
                    || target.isInsideOfMaterial(Material.water)
                ...
                if (dist < 1f && !(target instanceof EntityPlayer
                        && BeeManager.armorApiaristHelper.wearsItems(target, "computronics:swarm", true) >= 4)) {
                    target.attackEntityFrom(
                            !aggressive ? beeDamageSource : beeDamageSourceSelf,
                            getAmplifier() + (aggressive ? 1F : 0F));
                }
            }
            ...
            if (player.getHeldItem() != null && player.isBlocking()) {
                ...
                moveTo(
                        ...
                        player.posY + look.yCoord,
                        player.posZ + look.zCoord,
                        player.width / 2f,
                        ((player.height / 2f) + player.getEyeHeight()) / 2f,
                        0.3f,
                        1f,
                        0.5f);
            } else {
    ...
    private double circle(EntityLivingBase target, float yOffset, float modifier, float xFuzzy, float radius) {
        final Vec3 direction;
        {
            ...
            final Vec3 betweenX = Vec3.createVectorHelper(
                    between.xCoord / Math.abs(between.xCoord),
                    between.yCoord / Math.abs(between.xCoord),
                    between.zCoord / Math.abs(between.xCoord));
            final double normdist = betweenX.lengthVector();
            Vec3 targetRadius = Vec3.createVectorHelper(
                    betweenX.xCoord * radius / normdist,
                    betweenX.yCoord * radius / normdist,
                    betweenX.zCoord * radius / normdist);
            {
        ...
        double res = direction.xCoord * direction.xCoord + direction.yCoord * direction.yCoord
    ...
    private double moveTo(double x, double y, double z, double xzOffset, double yOffset, float modifier, float xFuzzy,
            float xFuzzyAttack) {
        ...
        /*
         * double ndeltaX = maxAbs(deltaX, Math.signum(deltaX), 0.1) / nres * modifier; double ndeltaY = maxAbs(deltaY,
         * Math.signum(deltaY), 0.1) / nres * modifier; double ndeltaZ = maxAbs(deltaZ, Math.signum(deltaZ), 0.1) / nres
         * * modifier; motionX += minAbs(ndeltaX, Math.signum(ndeltaX), 0.5); motionY += minAbs(ndeltaY,
         * Math.signum(ndeltaY), 0.5); motionZ += minAbs(ndeltaZ, Math.signum(ndeltaZ), 0.5);
         */
        if (res < 3f * 3f) {
            xFuzzy = xFuzzyAttack;
        }
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        return !(damageTypesImmune.contains(source.getDamageType())
                || (this.player != null && source.getEntity() == player))
    ...
    public void setDead() {
        super.setDead();
        /*
         * for(Entity member : swarmMembers) { member.setDead(); }
         */
    }
    ...
    protected void collideWithEntity(Entity entity) {
        /*
         * if(!worldObj.isRemote && player != null && entity != player && player.getHeldItem() != null &&
         * player.isBlocking()) { //super.collideWithEntity(entity); }
         */
    }
    ...
    public void applyEntityCollision(Entity entity) {
        if (!worldObj.isRemote && player != null
            ...
            /*
             * float health = getHealth(); float maxHealth = getMaxHealth(); float diff = health / maxHealth;
             * entity.motionX /= 100f * diff; entity.motionZ /= 100f * diff;
             */
    ...
    public static class BeeDamageSource extends DamageSource {
        ...
        public IChatComponent func_151519_b(EntityLivingBase victim) {
            ...
            String format = "death.attack." + this.damageType
                    + (numCauses > 1 ? "." + (victim.worldObj.rand.nextInt(this.numCauses) + 1) : "");
        ...
        /*
         * public BeeDamageSource(EntityLivingBase entity) { this(); this.entity = entity; }
