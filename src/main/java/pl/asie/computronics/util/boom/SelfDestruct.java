        ...
        if (this.isSmoking) {
            ...
            for (Object affectedBlockPosition : this.affectedBlockPositions) {
                ...
                if (block.getMaterial() != Material.air) {
                    if (!this.worldObj.isRemote && i == MathHelper.floor_double(explosionX)
                    ...
                    if (destroyBlocks) {
                        if (i != MathHelper.floor_double(explosionX) || j != MathHelper.floor_double(explosionY)
                                || k != MathHelper.floor_double(explosionZ)) {
        ...
        for (Object playerEntity : world.playerEntities) {
            if (playerEntity instanceof EntityPlayerMP) {
                ...
                if (entityplayer.getDistanceSq(xPos, yPos, zPos) < 4096.0D) {
                    try {
                        Packet p = Computronics.packet.create(PacketType.COMPUTER_BOOM.ordinal()).writeDouble(xPos)
                                .writeDouble(yPos).writeDouble(zPos).writeFloat(4.0F)
