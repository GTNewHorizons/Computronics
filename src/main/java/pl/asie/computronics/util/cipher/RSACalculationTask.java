...
public class RSACalculationTask implements Callable<ArrayList<Map<Integer, String>>> {
    ...
    private ArrayList<Map<Integer, String>> createKeySet(int keylength) {
        ...
        if (pair == null || !(pair.getPublic() instanceof RSAPublicKey)
                || !(pair.getPrivate() instanceof RSAPrivateKey)) {
            return null;
        }
    ...
    private static final BigInteger ONE = BigInteger.ONE, TWO = new BigInteger("2"), SEVENTEEN = new BigInteger("17");

    private ArrayList<Map<Integer, String>> createKeySet(int p, int q) {
        return this.createKeySet(BigInteger.valueOf(p), BigInteger.valueOf(q));
    }
