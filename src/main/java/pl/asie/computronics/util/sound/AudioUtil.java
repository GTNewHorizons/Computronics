...
public class AudioUtil {
    ...
    public static class ADSR {
        ...
        public ADSR(int attackDuration, int decayDuration, float attenuation, int releaseDuration) {
            ...
            this.decaySpeed = this.decayDuration == 0 ? Double.POSITIVE_INFINITY
                    : ((this.attenuation - 1D) * 1000D) / (this.decayDuration * Config.SOUND_SAMPLE_RATE);
            this.releaseSpeed = this.releaseDuration == 0 ? Double.NEGATIVE_INFINITY
                    : (-this.attenuation * 1000D) / (this.releaseDuration * Config.SOUND_SAMPLE_RATE);
        }
