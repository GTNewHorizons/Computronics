...
public class Audio {
    ...
    public void play(float x, float y, float z, String pattern, AudioType type, float frequencyInHz,
            int durationInMilliseconds, int initialDelayInMilliseconds) {
        ...
        if (disableAudio) {
            ...
            for (char ch : pattern.toCharArray()) {
                PositionedSoundRecord record = new PositionedSoundRecord(
                        new ResourceLocation("note.harp"),
                        distanceBasedGain * gain,
                        clampedFrequency,
                        x,
                        y,
                        z);
            ...
            if (AL.isCreated()) {
                ...
                for (int i = 0; i < chars.length; i++) {
                    sampleCounts[i] = (chars[i] == '.' ? durationInMilliseconds : 2 * durationInMilliseconds)
                            * sampleRate
                            / 1000;
                }
                ...
                ByteBuffer data = BufferUtils
                        .createByteBuffer(initialDelay + sampleSum + (sampleCounts.length - 1) * pauseSampleCount);
                ...
                try {
                    ...
                    if (e.errorCode == AL10.AL_OUT_OF_MEMORY) {
                        ...
                        Computronics.log.info(
                                "Couldn't play computer speaker sound because your sound card ran out of memory. Either your sound card is just really low-end, or there are just too many sounds in use already by other mods. Disabling computer speakers to avoid spamming your log file now.");
                        disableAudio = true;
                    } else {
    ...
    private class Source {
        ...
        Source(float x, float y, float z, ByteBuffer data, float gain) {
            ...
            for (int i = 0; i * sampleRate < data.capacity(); i++) {
                /*
                 * ByteBuffer buf = ByteBuffer.allocateDirect(Math.min(2 * sampleRate, bytes.length - (i * 2 *
                 * sampleRate))); buf.put(bytes, i * 2 * sampleRate, buf.capacity()); buf.rewind(); bufferData.add(buf);
                 */
            ...
            try {
                ...
                try {
                    ...
                    AL10.alSourcef(source, AL10.AL_ROLLOFF_FACTOR, (24F * 0.25F) / maxDistance); // At a distance of 24,
                                                                                                 // a rolloff factor of
                                                                                                 // 0.25 sounds good
                                                                                                 // enough.
                    checkALError();
