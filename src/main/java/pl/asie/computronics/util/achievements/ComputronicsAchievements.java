...
public class ComputronicsAchievements {
    ...
    public void initialize() {
        ...
        AchievementPage.registerAchievementPage(
                new AchievementPage(
                        "Computronics",
                        (Achievement[]) this.achievementMap.values()
                                .toArray(new Achievement[this.achievementMap.size()])));
    ...
    private void initializeAchievements() {
        if (Computronics.itemTape != null) {
            this.registerAchievement(
                    ...
                    true);
            this.registerAchievement(
                    EnumAchievements.Tape_Star,
                    4,
                    0,
                    new ItemStack(Computronics.itemTape, 1, 8),
                    this.getAchievement(EnumAchievements.Tape),
                    false,
                    false);
            ...
            if (Mods.hasVersion(Mods.GregTech, Mods.Versions.GregTech5)) {
                this.registerAchievement(
                        ...
                        this.getAchievement(EnumAchievements.Tape_Star),
                        true,
                        false);
                this.registerAchievement(
                        EnumAchievements.Tape_IG_Dropped,
                        8,
                        10,
                        ItemList.IC2_Scrap.get(1),
                        this.getAchievement(EnumAchievements.Tape_IG),
                        true,
                        false);
    ...
    private Achievement registerAchievement(EnumAchievements key, int x, int y, ItemStack icon, Achievement requirement,
            boolean special, boolean independent) {
        Achievement achievement = new Achievement(
                "computronics." + key.getKey(),
                "computronics." + key.getKey(),
                x,
                y,
                icon,
    ...
    public void onItemDropped(ItemTossEvent event) {
        ...
        if (stack != null && Computronics.itemTape != null
                && stack.getItem() == Computronics.itemTape
                    ...
                    if (playername != null && !playername.isEmpty()) {
                        for (Object o : MinecraftServer.getServer().getConfigurationManager().playerEntityList) {
                            if (o instanceof EntityPlayer
                                    && ((EntityPlayer) o).getCommandSenderName().equals(playername)) {
    ...
    private static class RailcraftAchievements {
        ...
        private static void initializeRCAchievements() {
            Computronics.instance.achievements.registerAchievement(
                    EnumAchievements.Locomotive,
                    0,
                    4,
                    EnumCart.LOCO_ELECTRIC.getCartItem(),
                    null,
                    false,
                    true);
            Computronics.instance.achievements.registerAchievement(
                    EnumAchievements.Relay,
                    2,
                    6,
                    new ItemStack(Computronics.railcraft.relaySensor),
                    Computronics.instance.achievements.getAchievement(EnumAchievements.Locomotive),
                    false,
                    false);
        }

        private static void onCrafting(ItemStack stack, EntityPlayer player) {
            if (stack.getItem() instanceof ItemLocomotive && ItemLocomotive.getModel(stack)
                    .equals(ItemLocomotive.getModel(EnumCart.LOCO_ELECTRIC.getCartItem()))) {
        ...
        private static void onLeftClickEntity(AttackEntityEvent event) {
            if (Mods.isLoaded(Mods.Railcraft) && event != null
                    && event.target != null
                    ...
                    if (data != null && data.hasKey("bound") && data.getBoolean("bound")) {
                        ...
                        if (loco.worldObj.getTileEntity(x, y, z) != null
                            ...
                            if (loco.dimension == relay.getWorldObj().provider.dimensionId
                                    && loco.getDistanceSq(relay.xCoord, relay.yCoord, relay.zCoord)
