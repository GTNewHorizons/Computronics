...
public class GuiProviderCipher extends LegacyGuiProvider {
    ...
    protected GuiBase makeGuiBase(int i, EntityPlayer entityPlayer, World world, int x, int y, int z,
            TileEntityBase tile) {
        return new GuiCipherBlock(makeContainerBase(guiID, entityPlayer, world, x, y, z, tile));
    }

    @Override
    protected ContainerBase makeContainerBase(int i, EntityPlayer entityPlayer, World world, int x, int y, int z,
            TileEntityBase tile) {
