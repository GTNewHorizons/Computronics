...
public class SoundCardPacket extends AudioPacket {
    ...
    protected void writeData(Packet packet) throws IOException {
        packet.writeString(address).writeInt(instructions.size());
        for (Instruction instruction : instructions) {
            if (instruction instanceof Open) {
                packet.writeByte((byte) 0).writeByte((byte) ((Open) instruction).channelIndex);
            } else if (instruction instanceof Close) {
                packet.writeByte((byte) 1).writeByte((byte) ((Close) instruction).channelIndex);
            } else if (instruction instanceof SetWave) {
                packet.writeByte((byte) 2).writeByte((byte) ((SetWave) instruction).channelIndex)
                        .writeInt(((SetWave) instruction).type.ordinal());
            } else if (instruction instanceof Delay) {
                packet.writeByte((byte) 3).writeInt(((Delay) instruction).delay);
            } else if (instruction instanceof SetFM) {
                packet.writeByte((byte) 4).writeByte((byte) ((SetFM) instruction).channelIndex)
                ...
                packet.writeByte((byte) 5).writeByte((byte) ((ResetFM) instruction).channelIndex);
            } else if (instruction instanceof SetAM) {
                packet.writeByte((byte) 6).writeByte((byte) ((SetAM) instruction).channelIndex)
                        .writeInt(((SetAM) instruction).ampMod.modulatorIndex);
            } else if (instruction instanceof ResetAM) {
                packet.writeByte((byte) 7).writeByte((byte) ((ResetAM) instruction).channelIndex);
            } else if (instruction instanceof SetADSR) {
                ADSR envelope = ((SetADSR) instruction).envelope;
                packet.writeByte((byte) 8).writeByte((byte) ((SetADSR) instruction).channelIndex)
                        .writeInt(envelope.attackDuration).writeInt(envelope.decayDuration)
                        .writeFloat(envelope.attenuation).writeInt(envelope.releaseDuration);
            } else if (instruction instanceof ResetEnvelope) {
                packet.writeByte((byte) 9).writeByte((byte) ((ResetEnvelope) instruction).channelIndex);
            } else if (instruction instanceof SetVolume) {
                packet.writeByte((byte) 10).writeByte((byte) ((SetVolume) instruction).channelIndex)
                        .writeFloat(((SetVolume) instruction).volume);
            } else if (instruction instanceof SetFrequency) {
                packet.writeByte((byte) 11).writeByte((byte) ((SetFrequency) instruction).channelIndex)
                        .writeFloat(((SetFrequency) instruction).frequency);
            } else if (instruction instanceof SetWhiteNoise) {
                packet.writeByte((byte) 12).writeByte((byte) ((SetWhiteNoise) instruction).channelIndex);
            } else if (instruction instanceof SetLFSR) {
                packet.writeByte((byte) 13).writeByte((byte) ((SetLFSR) instruction).channelIndex)
                        .writeInt(((SetLFSR) instruction).initial).writeInt(((SetLFSR) instruction).mask);
            }
        }
    }
}
