    ...
    public boolean preInit() {
        try {
            ...
            if (voice == null) {
                ...
                if (voice == null) {
                    log.error(
                            "No voice found for available locales, please install a matching voice file. Available locales: "
                                    + StringUtils.join(availableLocales, ", "));
            ...
            ttsThreads = Executors
                    .newFixedThreadPool(2, new ThreadFactoryBuilder().setPriority(Thread.MIN_PRIORITY).build());
            log.info(
                    "Text To Speech successfully initialized with language " + marytts.getLocale()
                            + " and voice "
                            + voice
                            + ".");
            return true;
