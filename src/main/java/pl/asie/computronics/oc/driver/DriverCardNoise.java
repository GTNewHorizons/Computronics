...
public class DriverCardNoise extends DriverCardSoundBase {
    ...
    public DriverCardNoise(EnvironmentHost host) {
        super(host, "play");
        this.setNode(Network.newNode(this, Visibility.Neighbors).withComponent("noise").withConnector().create());
    ...
    public void update() {
        ...
        if (channelSendBuffer != null) {
            try {
                sendSound(host.world(), host.xPosition(), host.yPosition(), host.zPosition(), channelSendBuffer);
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    @Callback(
            doc = "function(channel:number, mode:number):boolean; Sets the audio mode of the specified channel.",
            direct = true)
    ...
    @Callback(
            doc = "function(channel:number, frequency:number, duration:number [, initialDelay:number]):boolean; Adds the frequency played with the duration to the channel's buffer, optionally with a delay before playing. Returns true on success.",
            direct = true,
            limit = 50)
    ...
    @Callback(
            doc = "function():boolean; Starts processing the buffer and clears it. Returns true on success.",
            direct = true,
            limit = 10)
    public Object[] process(Context context, Arguments args) {
        ...
        for (int i = 0; i < channels.length; i++) {
            ...
            for (Channel.WaveEntry entry : channel.entries) {
                ...
                channelSendBuffer[i].addWaveEntry(
                        entry.freqPair.frequency,
                        entry.freqPair.duration,
                        entry.initialDelay + totalDelay);
                totalDelay += entry.initialDelay + entry.freqPair.duration;
            }
            longest = Math.max(longest, Math.max(50, Math.min(5000, (totalDelay / 1000D))));
        }
        Object[] error = tryConsumeEnergy(
                Config.BEEP_ENERGY_COST * getNonNullCount(channelSendBuffer) * longest,
                "process");
        if (error != null) {
            channelSendBuffer = null;
            return error;
    ...
    @Callback(
            doc = "function(channels:table):boolean; table must have 8 or fewer entries. Each entry must be a table containing a frequency and a duration as values; plays each frequency for the specified duration. Returns true on success.",
            direct = true,
            limit = 1)
    public Object[] play(Context context, Arguments args) throws Exception {
        ...
        for (int channel = 1; channel <= 8; channel++) {
            ...
            if (!(freqObj instanceof Number)) {
                throw new IllegalArgumentException(
                        "frequency " + String.valueOf(freqObj)
                                + " on channel "
            ...
            if (durObj != null && !(durObj instanceof Number)) {
                throw new IllegalArgumentException(
                        "duration '" + String.valueOf(durObj)
                                + " on channel "
                                + String.valueOf(channel)
                                + " is not a number");
            }
        ...
        return tryQueueSound(
                freqPairs,
                new Object[] { true },
                Config.BEEP_ENERGY_COST * getNonNullCount(freqPairs) * (longest / 1000D),
                playMethodName);
    }
    ...
    public void load(NBTTagCompound nbt) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            channels[i].entries.add(
                    new Channel.WaveEntry(new Channel.FreqPair(frequencies.func_150308_e(i), durations[i]), delays[i]));
        }
    }
    ...
    protected OCUtils.Device deviceInfo() {
        return new OCUtils.Device(DeviceClass.Multimedia, "Audio interface", OCUtils.Vendors.Yanaki, "SQ1289-4");
    }
    ...
    protected void sendSound(World world, double x, double y, double z, Channel[] channels) throws Exception {
        ...
        Packet packet = Computronics.packet.create(PacketType.COMPUTER_NOISE.ordinal())
                .writeInt(world.provider.dimensionId).writeFloat((float) x).writeFloat((float) y).writeFloat((float) z)
                .writeByte(hits);
        for (Channel channel : channels) {
            if (channel != null) {
                ...
                for (int i = 0; i < channel.entries.size(); i++) {
                    ...
                    if (entry != null && entry.freqPair != null) {
                        packet.writeByte((byte) getMode(i).ordinal()).writeFloat(entry.freqPair.frequency)
                                .writeShort((short) entry.freqPair.duration).writeShort((short) entry.initialDelay);
                    }
                }
            }
        }
        Computronics.packet.sendToAllAround(
                packet,
                new NetworkRegistry.TargetPoint(world.provider.dimensionId, x, y, z, Config.SOUND_RADIUS));
    }
