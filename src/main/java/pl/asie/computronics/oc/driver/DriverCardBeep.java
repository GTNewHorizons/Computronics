...
public class DriverCardBeep extends DriverCardSoundBase {
    ...
    public DriverCardBeep(EnvironmentHost host) {
        super(host, "beep");
        this.setNode(Network.newNode(this, Visibility.Neighbors).withComponent("beep").withConnector().create());
    }
    ...
    @Callback(
            doc = "function(frequencyDurationTable:table):boolean; table needs to contain frequency-duration pairs; plays each frequency for the specified duration. Returns true on success.",
            direct = true,
            limit = 10)
    ...
    protected OCUtils.Device deviceInfo() {
        return new OCUtils.Device(DeviceClass.Multimedia, "Audio interface", OCUtils.Vendors.Yanaki, "SQ532");
    }
}
