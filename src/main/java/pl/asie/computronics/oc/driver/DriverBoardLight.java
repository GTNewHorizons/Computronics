...
public class DriverBoardLight extends RackMountableWithComponentConnector {
    ...
    public DriverBoardLight(Rack host) {
        this.host = host;
        this.setNode(
                Network.newNode(this, Visibility.Network).withComponent("light_board", Visibility.Network)
    ...
    public enum Mode {
        ...
        Mode(int lightcount) {
            ...
            this.foreground = new ResourceLocation(
                    "computronics",
                    "textures/blocks/light_board/mode_" + index + "_lights.png");
    ...
    @Callback(
            doc = "function(index:number, color:number):boolean; Sets the color of the specified light. Returns true on success, false and an error message otherwise",
            direct = true)
    ...
    @Callback(
            doc = "function(index:number):number; Returns the color of the specified light on success, false and an error message otherwise",
            direct = true)
    ...
    @Callback(
            doc = "function(index:number, active:boolean):boolean; Turns the specified light on or off. Returns true on success, false and an error message otherwise",
            direct = true)
    ...
    @Callback(
            doc = "function(index:number):boolean; Returns true if the light at the specified position is currently active",
            direct = true)
    ...
    @Callback(
            value = "light_count",
            doc = "This represents the number of lights on the board.",
            direct = true,
            getter = true)
    ...
    public boolean onActivate(EntityPlayer player, float hitX, float hitY) {
        ...
        final int x = MathHelper.floor_double(host.xPosition()), y = MathHelper.floor_double(host.yPosition()),
                z = MathHelper.floor_double(host.zPosition());
        ItemStack held = player.getHeldItem();
        if (held != null && held.getItem() != null
                && Integration.isTool(held, player, x, y, z)
                && Integration.useTool(held, player, x, y, z)) {
    ...
    protected OCUtils.Device deviceInfo() {
        return new OCUtils.Device(DeviceClass.Display, "Light board", OCUtils.Vendors.Lumiose, "LED-15 X");
    }
