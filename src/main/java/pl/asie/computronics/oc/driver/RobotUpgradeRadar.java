...
public class RobotUpgradeRadar extends ManagedEnvironment implements DeviceInfo {
    ...
    public RobotUpgradeRadar(EnvironmentHost container) {
        ...
        this.setNode(
                Network.newNode(this, Visibility.Network)
                        .withConnector(Config.RADAR_ENERGY_COST_OC * Config.RADAR_RANGE * 1.75)
                        .withComponent("radar", Visibility.Neighbors).create());
    }
    ...
    private AxisAlignedBB getBounds(int d) {
        ...
        return AxisAlignedBB.getBoundingBox(
                container.xPosition() - 0.5,
                container.yPosition() - 0.5,
                container.zPosition() - 0.5,
                container.xPosition() + 0.5,
                container.yPosition() + 0.5,
                container.zPosition() + 0.5).expand(distance, distance, distance);
    }
    ...
    public Map<String, String> getDeviceInfo() {
        if (deviceInfo == null) {
            return deviceInfo = new OCUtils.Device(
                    ...
                    "Detectotron M1 Mini").deviceInfo();
        }
        return deviceInfo;
    }
}
