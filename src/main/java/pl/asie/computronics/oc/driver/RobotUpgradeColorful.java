...
public class RobotUpgradeColorful extends ManagedEnvironmentWithComponentConnector {
    ...
    public RobotUpgradeColorful(EnvironmentHost host) {
        this.host = host;
        this.setNode(
                Network.newNode(this, Visibility.Network).withConnector().withComponent("colors", Visibility.Neighbors)
                        .create());
    }

    @Callback(
            doc = "function(color:number):boolean; Sets the color of the robot. Returns true on success, false and an error message otherwise",
            direct = true)
    ...
    @Callback(
            doc = "function():boolean; Resets the colour of the robot. Returns true on success, false and an error message otherwise",
            direct = true)
    ...
    protected OCUtils.Device deviceInfo() {
        return new OCUtils.Device(DeviceClass.Display, "Color overlay", OCUtils.Vendors.Lumiose, "Holonaut H4-1463 v2");
    }
}
