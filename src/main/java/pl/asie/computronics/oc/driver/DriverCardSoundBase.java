...
public abstract class DriverCardSoundBase extends ManagedEnvironment implements DeviceInfo {
    ...
    public void update() {
        ...
        if (sendBuffer != null) {
            try {
                sendSound(host.world(), host.xPosition(), host.yPosition(), host.zPosition(), sendBuffer);
            } catch (Exception e) {
                e.printStackTrace();
            }
            sendBuffer = null;
    ...
    protected Object[] tryQueueSound(Channel.FreqPair[] freqPairs, Object[] result, double v, String methodName)
            throws Exception {
    ...
    protected void sendSound(World world, double x, double y, double z, Channel.FreqPair[] freqPairs) throws Exception {
        ...
        for (int i = 0; i < freqPairs.length; i++) {
            ...
            if (freqPair != null) {
                packet.writeByte((byte) getMode(i).ordinal()).writeFloat(freqPair.frequency)
