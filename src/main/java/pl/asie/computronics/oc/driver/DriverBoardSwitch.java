...
public class DriverBoardSwitch extends RackMountableWithComponentConnector {
    ...
    public DriverBoardSwitch(Rack host) {
        this.host = host;
        this.setNode(
                Network.newNode(this, Visibility.Network).withComponent("switch_board", Visibility.Network)
    ...
    @Callback(
            doc = "function(index:number, active:boolean):boolean; Activates or deactivates the specified switch. Returns true if the state changed.",
            direct = true)
    ...
    @Callback(
            doc = "function(index:number):boolean; Returns true if the switch at the specified position is currently active",
            direct = true)
    ...
    protected OCUtils.Device deviceInfo() {
        return new OCUtils.Device(DeviceClass.Input, "Switch board", OCUtils.Vendors.Soluna, "Clickety-Clack Q3");
    }
}
