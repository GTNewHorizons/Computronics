...
public class DriverCardSpoof extends NetworkCard {
    ...
    public DriverCardSpoof(EnvironmentHost container) {
        ...
        this.setNode(
                Network.newNode(this, Visibility.Network).withComponent("modem", Visibility.Neighbors).withConnector()
                        .create());
    }
    ...
    public Map<String, String> getDeviceInfo() {
        if (deviceInfo == null) {
            return deviceInfo = new OCUtils.Device(
                    ...
                    "42i520 (MPN-01) - Br1ck").deviceInfo();
        }
        return deviceInfo;
    }
    ...
    @Override
    @Callback(
            doc = "function(targetaddress:string, [sourceaddress:string,] port:number, data...) -- Sends the specified data to the specified target (from the source address if specified)")
    ...
    @Override
    @Callback(
            doc = "function([sourceaddress:string,] port:number, data...) -- Broadcasts the specified data on the specified port (from the source address if specified)")
