...
public class RobotUpgradeChatBox extends ManagedEnvironment implements DeviceInfo, IChatListener {
    ...
    public RobotUpgradeChatBox(EnvironmentHost container) {
        ...
        this.setNode(
                Network.newNode(this, Visibility.Network).withConnector().withComponent("chat", Visibility.Neighbors)
                        .create());
    }
    ...
    public void receiveChatMessage(ServerChatEvent event) {
        if (!Config.CHATBOX_MAGIC && (event.player.worldObj != this.container.world()
                || event.player.getDistanceSq(container.xPosition(), container.yPosition(), container.zPosition())
                        > distance * distance)) {
            return;
        }
    ...
    @Callback(
            doc = "function(text:string [, distance:number]):boolean; "
                    + "Makes the robot say some text with the currently set or the specified distance. Returns true on success",
            direct = true,
            limit = 3)
    public Object[] say(Context context, Arguments args) {
        if (args.count() >= 1) {
            ...
            // if(args.isString(0)) ChatBoxUtils.sendChatMessage(this.container, Computronics.CHATBOX_DISTANCE,
            // "ChatBox", args.checkString(0));
            int d = distance;
            if (args.count() >= 1) {
                if (args.isInteger(1)) {
