...
public class DriverCardBoom extends ManagedEnvironmentWithComponentConnector {
    ...
    protected void createNode() {
        this.setNode(Network.newNode(this, Visibility.Neighbors).withComponent("self_destruct").create());
    }
    ...
    @Callback(
            doc = "function([time:number]):number; Starts the countdown; Will be ticking down until the time is reached. 5 seconds by default. Returns the time set")
    ...
    protected void goBoom() {
        SelfDestruct
                .goBoom(container.world(), container.xPosition(), container.yPosition(), container.zPosition(), true);
    }
}
