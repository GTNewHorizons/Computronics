...
public class RobotUpgradeCamera extends ManagedEnvironment implements DeviceInfo {
    ...
    public RobotUpgradeCamera(EnvironmentHost entity) {
        this.entity = entity;
        this.setNode(
                Network.newNode(this, Visibility.Network).withConnector().withComponent("camera", Visibility.Neighbors)
                        .create());
    }
    ...
    private ForgeDirection getFacingDirection() {
        if (entity instanceof Rotatable) {
            ...
            int l = MathHelper.floor_double(
                    (double) (entity.world().getClosestPlayer(
                            entity.xPosition(),
                            entity.yPosition(),
                            entity.zPosition(),
                            1.0D).rotationYaw * 4.0F / 360.0F) + 0.5D)
                    & 3;
    ...
    @Callback(
            doc = "function([x:number, y:number]):number; "
                    ...
                    + "or of the block directly above",
            direct = true,
            limit = CALL_LIMIT)
    ...
    @Callback(
            doc = "function([x:number, y:number]):number; "
                    ...
                    + "or of the block directly below",
            direct = true,
            limit = CALL_LIMIT)
    ...
    public Map<String, String> getDeviceInfo() {
        if (deviceInfo == null) {
            return deviceInfo = new OCUtils.Device(
                    ...
                    "Compact Spatiometer 1-C").deviceInfo();
        }
        return deviceInfo;
    }
}
