...
public class IntegrationOpenComputers {
    ...
    public void preInit() {
        ...
        if (Mods.hasVersion(Mods.Forestry, Mods.Versions.Forestry)) {
            ...
            log.warn(
                    "Detected outdated version of Forestry, Forestry integration will not be enabled. Please update to Forestry "
                            + Mods.Versions.Forestry
                            + " or later.");
        }

        if (Mods.isLoaded(Mods.BuildCraftTransport) && Mods.isLoaded(Mods.BuildCraftCore)
                && Config.BUILDCRAFT_STATION) {
    ...
    private static class ReadOnlyFS implements Callable<FileSystem> {
        ...
        public FileSystem call() throws Exception {
            return li.cil.oc.api.FileSystem.asReadOnly(
                    li.cil.oc.api.FileSystem.fromClass(Computronics.class, Mods.Computronics, "loot/" + name));
        }
    }
    ...
    public void init() {
        ...
        if (Mods.API.hasAPI(Mods.API.DraconicEvolution) && compat.isCompatEnabled(Compat.DraconicEvolution)) {
            Driver.add(new DriverExtendedRFStorage.OCDriver());
        }

        if (Mods.API.hasAPI(Mods.API.Mekanism_Energy) && compat.isCompatEnabled(Compat.MekanismEnergy)) {
    ...
    public void postInit() {
        ...
        if (Config.OC_CARD_BEEP) {
            if (!Computronics.isGTNHLoaded) {
                RecipeUtils.addShapedRecipe(
                        ...
                        "oc:circuitChip1",
                        'f',
                        speaker != null ? speaker : ironNote != null ? ironNote : Blocks.noteblock,
                        'b',
                        "oc:materialCard",
                        'l',
                        "oc:materialCU");
        ...
        if (Config.OC_UPGRADE_COLORFUL) {
            if (!Computronics.isGTNHLoaded) {
                RecipeUtils.addShapedRecipe(
                        new ItemStack(itemOCParts, 1, 7),
                        "fdf",
                        "mcm",
                        "fbf",
                        'c',
                        colorfulLamp != null ? colorfulLamp : "glowstone",
                        'd',
                        "oc:materialTransistor",
                        'm',
                        "oc:circuitChip2",
        ...
        if (Config.OC_CARD_NOISE) {
            if (!Computronics.isGTNHLoaded) {
                RecipeUtils.addShapedRecipe(
                        ...
                        "oc:materialALU",
                        'b',
                        Config.OC_CARD_BEEP ? new ItemStack(itemOCParts, 1, 5)
                                : speaker != null ? speaker : ironNote != null ? ironNote : Blocks.noteblock,
                        'l',
                        "oc:circuitChip2",
                        'n',
        ...
        if (Config.OC_CARD_SOUND) {
            if (!Computronics.isGTNHLoaded) {
                RecipeUtils.addShapedRecipe(
                        ...
                        "oc:cpu1",
                        'b',
                        Config.OC_CARD_NOISE ? new ItemStack(itemOCParts, 1, 8)
                                : Config.OC_CARD_BEEP ? new ItemStack(itemOCParts, 1, 5)
                                        : speaker != null ? speaker : ironNote != null ? ironNote : Blocks.noteblock,
                        'l',
                        "oc:circuitChip2");
        ...
        if (Config.OC_BOARD_LIGHT) {
            if (!Computronics.isGTNHLoaded) {
                RecipeUtils.addShapedRecipe(
                        new ItemStack(itemOCParts, 1, 10),
                        "oso",
                        "gcg",
                        "opo",
                        's',
        ...
        if (Config.OC_BOARD_BOOM) {
            if (!Computronics.isGTNHLoaded) {
                RecipeUtils.addShapedRecipe(
                        new ItemStack(itemOCParts, 1, 11),
                        "lsl",
                        "gcg",
                        "opo",
                        's',
                        "oc:circuitChip1",
                        'g',
                        Config.OC_CARD_BOOM ? new ItemStack(itemOCParts, 1, 6) : Items.blaze_powder,
                        'c',
                        Blocks.tnt,
                        'o',
                        "obsidian",
        ...
        if (Config.OC_BOARD_CAPACITOR) {
            if (!Computronics.isGTNHLoaded) {
                RecipeUtils.addShapedRecipe(
                        new ItemStack(itemOCParts, 1, 12),
                        "lsl",
                        "gcg",
                        "opo",
                        's',
        ...
        if (Config.OC_BOARD_SWITCH) {
            if (!Computronics.isGTNHLoaded) {
                RecipeUtils.addShapedRecipe(
                        new ItemStack(itemOCParts, 1, 13),
                        "oso",
                        "gcg",
                        "opo",
                        's',
        ...
        if (Config.OC_UPGRADE_SPEECH) {
            if (speechBox != null) {
                if (!Computronics.isGTNHLoaded) {
                    RecipeUtils.addShapedRecipe(
                            new ItemStack(itemOCParts, 1, 14),
                            "idi",
                            "mcm",
                            "ibi",
                            'c',
