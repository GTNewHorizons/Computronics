...
public class UpgradeRenderer {
    ...
    private final ResourceLocation upgradeRadar = new ResourceLocation(
            "computronics",
            "textures/models/UpgradeRadar.png"),
            ...
            case 2: {
                if (mountPoint.name.equals(MountPointName.TopLeft) || mountPoint.name.equals(MountPointName.TopRight)) {
                    float degrees = robot.shouldAnimate()
                            ? ((robot.world().getTotalWorldTime() + (robot.hashCode() ^ 0xFF)) % 160 + pt) / 160F * 360F
                            : 0F;
    ...
    /*
     * @Optional.Method(modid = Mods.OpenComputers) private void drawSimpleBlock(MountPoint mountPoint) {
     * drawSimpleBlock(mountPoint, 0); }
     */

    /*
     * @Optional.Method(modid = Mods.OpenComputers) private void drawSimpleBlock(MountPoint mountPoint, float
     * frontOffset) { drawSimpleBlock(mountPoint, frontOffset, false); }
     */
